...
async function comparePassword(password, storedPassword) {
  try {
    // Check for plain text match first (for development/test data)
    ...
    try {
      ...
      console.log('⚠️ Bcrypt error (expected for plain text):', bcryptError.message);
    }
...
// Start server - listen on all interfaces (0.0.0.0) for Render deployment
